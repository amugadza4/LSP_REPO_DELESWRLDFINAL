...
Collaborators (if any): GroundStation
Assumptions (if any): Each aircraft is equipped with exactly one transponder; the broadcast packet format is standardized.
...
Collaborators (if any): Transponder, PacketDecoder, AircraftDatabase
Assumptions (if any): The ground station maintains continuous radio contact with all aircraft within the airport airspace.
...
Collaborators (if any): GroundStation, FlightRecord
Assumptions (if any): A single fixed high-density packet format is used; format versioning is out of scope.
...
Collaborators (if any): AircraftDatabase, PacketDecoder
Assumptions (if any): One FlightRecord exists per aircraft; created on first contact and removed when the aircraft leaves airspace.
...
Collaborators (if any): FlightRecord, GroundStation, DisplayManager, SafetyAnalyzer, ControllerInterface
Assumptions (if any): The database is maintained entirely in memory for real-time performance; persistence is out of scope.
...
Collaborators (if any): AircraftDatabase, SafetyAnalyzer, RadarDisplay
Assumptions (if any): The display refresh cycle is timer-driven at a fixed 10-second interval.
...
Collaborators (if any): DisplayManager
Assumptions (if any): RadarDisplay is a passive rendering surface; all decisions about what to show are made by DisplayManager.
...
Collaborators (if any): AircraftDatabase, FlightRecord, AlertSystem
Assumptions (if any): Safety analysis runs once per 10-second update cycle; continuous real-time analysis is out of scope.
...
Collaborators (if any): SafetyAnalyzer, ControllerInterface
Assumptions (if any): Alert acknowledgment by the controller is recorded but does not suppress continued safety analysis.
...
Collaborators (if any): AircraftDatabase, AlertSystem
Assumptions (if any): The controller interacts via a single workstation terminal; simultaneous multi-user access is out of scope.
